double getInstrAxis1() {
  ...
  double p=(double)((long)p1+indexAxis1Steps)/(double)StepsPerDegreeAxis1;
  cli(); long p2=posAxis2; sei();
  double q=(double)((long)p2+indexAxis2Steps)/(double)StepsPerDegreeAxis2;
...
void setIndexAxis1(double axis1, int newPierSide) {
  ...
  cli(); long p1=posAxis1; sei(); // sky=pos+index,  index=sky-pos
  indexAxis1=axis1-(double)p1/(double)StepsPerDegreeAxis1;
  indexAxis1Steps=(long)(indexAxis1*(double)StepsPerDegreeAxis1);
}

void setIndexAxis2(double axis2, int newPierSide) {
  if (latitude>=0) {
    if (newPierSide==PierSideWest) axis2=180.0-axis2;
  } else {
    if (newPierSide==PierSideWest) axis2=-180.0-axis2;
  }
  if (axis2>360.0) axis2-=360.0;
  if (axis2<-360.0) axis2+=360.0;
  cli(); long p2=posAxis2; sei(); // sky=pos+index,  index=sky-pos
  indexAxis2=axis2-(double)p2/(double)StepsPerDegreeAxis2;
  indexAxis2Steps=(long)(indexAxis2*(double)StepsPerDegreeAxis2);
}

void setTargetAxis1(double axis1, int newPierSide) {
  if (newPierSide==PierSideWest) axis1=axis1+180.0;
  long p1=(double)(axis1-indexAxis1)*(double)StepsPerDegreeAxis1;
...
void setTargetAxis2(double axis2, int newPierSide) {
  ...
  if (axis2>360.0) axis2-=360.0;
  if (axis2<-360.0) axis2+=360.0;
  long p2=(double)(axis2-indexAxis2)*(double)StepsPerDegreeAxis2;
