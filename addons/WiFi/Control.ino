...
const char html_controlStartStop1[] PROGMEM = "<button type='button' onpointerdown=\"s('";
const char html_controlStartStop2[] PROGMEM = "','1')\" >" L_START "</button><button type='button' onpointerdown=\"s('";
const char html_controlStartStop3[] PROGMEM = "','0')\" >" L_STOP "</button>\r\n";
...
const char html_controlExposure[] PROGMEM ="<input style='width: 12em; background: #111' type='range' min='0' max='255' value='";
const char html_controlDelay[] PROGMEM ="<input style='width: 12em; background: #111' type='range' min='10' max='255' value='";
const char html_controlCount[] PROGMEM ="<input style='width: 12em; background: #111' type='range' min='1' max='255' value='";
  ...
  if (mountStatus.featureFound()) {
    ...
    for (int i=0; i<8; i++) {
      ...
      if (mountStatus.featurePurpose() == INTERVALOMETER) {
        ...
        data += "&bull;";
        data += F("</div><div style='float: left; width: 14em; height: 2em; line-height: 2em'>");
        data += FPSTR(html_controlStartStop1);
        sprintf(temp1,"x%dv1",i+1);
        data += temp1;
        data += FPSTR(html_controlStartStop2);
        sprintf(temp1,"x%dv1",i+1);
        data += temp1;
